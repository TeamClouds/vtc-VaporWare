...
for i in "${LIST[@]}"; do
    ...
    convert -font $FONT +antialias -trim -pointsize $SIZE -rotate 270 -flip -endian LSB label:"$i" xbm:- | grep -o '0x\w\w' | awk '{ if (NR%8==0) printf $1 ", "; else printf $1 ", "; }'
    printf "\n\n"
done
...
MAXHEIGHT=1
for i in "${LIST[@]}"; do
    WIDTH=`convert -font $FONT +antialias -trim -pointsize $SIZE -rotate 270 -flip -endian LSB label:"$i" xbm:- | grep "height" | cut -d ' ' -f3`
    HEIGHT=`convert -font $FONT +antialias -trim -pointsize $SIZE -rotate 270 -flip -endian LSB label:"$i" xbm:- | grep "width" | cut -d ' ' -f3`
    BYTESIZE=`convert -font $FONT +antialias -trim -pointsize $SIZE -rotate 270 -flip -endian LSB label:"$i" xbm:- | grep -o "," | wc -l`
